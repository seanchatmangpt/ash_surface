...
defmodule AshSurface.AshSectionTruthTest do
  @moduledoc """
  TRUTH tests for the `ash` section: `AshSurface.Compiler.Ash` compiling a rich
  ...
  defp section_for(action) do
    assert {:ok, ir} = Ash.build(@gadget, action)
    assert {:ok, section} = Ash.section(ir, [])
    section
  end
  ...
  test "every section fact is inside the admitted truth vocabulary" do
    vocabulary =
      MapSet.new(
        Ash.admitted_keys().section ++
          Ash.admitted_keys().input ++
          Ash.admitted_keys().output ++
          Ash.admitted_keys().policy ++ [:check, :kind, :opts]
      )
  ...
  test "admission law: unknown and non-public actions are refused, never guessed" do
    ...
    assert {:error, {:unknown_public_action, :hidden, @public_actions}} =
             Ash.build(@gadget, :hidden)

    assert {:error, {:not_an_ash_resource, Enum}} = Ash.build(Enum, :search)
  end
